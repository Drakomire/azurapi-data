    ...
    for (let id of Object.keys(groups)) {
      ...
      for (let id of Object.keys(ships)) {
        ...
        if (lookup_table[stat.name.toLowerCase()] === undefined){
          ...
          if (!stat.tag_list.includes("\u03bc")) lookup_table[stat.english_name.replace(/^.[A-Z]+ /,'').toLowerCase()] = (id-id%10)/10;
        }
        ...
        if (compiled[ship.group_type].skin_thumbnails === undefined){
          ...
          for (skin_id of skin_ids){
            ...
            skin_portaits.push(`https://raw.githubusercontent.com/Drakomire/perseus-data/master/AzurLaneImages/assets/artresource/atlas/squareicon/${skin.painting}.png`)
          }
